...
# Description: Helper functions for downloading and uploading Telegram files
# Author: Gagan
...
from config import LOG_GROUP, OWNER_ID, STRING, API_ID, API_HASH
...
async def fetch_upload_method(user_id):
    ...
    return user_upload_methods.get(user_id, "Pyrogram")
        ...
        if 't.me/c/' in msg_link or 't.me/b/' in msg_link:
            ...
            if chat in saved_channel_ids:
                await app.edit_message_text(
                    ...
                    "Sorry! This channel is protected by **__ㅤ@BADDY_15__**."
                )
                return
...
user_delete_words = {}
user_replacement_words = {}

def load_delete_words(user_id):
    return user_delete_words.get(user_id, set())

def save_delete_words(user_id, words):
    user_delete_words[user_id] = words

def load_replacement_words(user_id):
    return user_replacement_words.get(user_id, {})

def save_replacement_words(user_id, replacements):
    user_replacement_words[user_id] = replacements

async def set_rename_command(user_id, custom_rename_tag):
    user_rename_preferences[str(user_id)] = custom_rename_tag

def get_user_rename_preference(user_id):
    return user_rename_preferences.get(str(user_id), '@BADDY_15')
...
async def send_settings_message(chat_id, user_id):
    buttons = [
        ...
        [Button.url("Report Errors", "https://t.me/BADDY_15")]
    ]
...
async def callback_query_handler(event):
    ...
    if event.data == b'setchat':
        ...
        current_method = user_upload_methods.get(user_id, 'Pyrogram')
        ...
        buttons = [
            ...
            [Button.inline(f"Rank Raiders v1 ⚡{telethon_check}", b'telethon')]
        ]
        await event.edit("Choose your preferred upload method:\n\n__**Note:** **BADDY ⚡**, built on Telethon(base), by @BADDY_15 still in beta.__", buttons=buttons)

    elif event.data == b'pyrogram':
        user_upload_methods[user_id] = "Pyrogram"
        await event.edit("Upload method set to **Pyrogram** ✅")

    elif event.data == b'telethon':
        user_upload_methods[user_id] = "Telethon"
        await event.edit("Upload method set to **Rank Raiders ⚡\n\nThanks for choosing this library as it will help me to analyze the error raise issues on github.** ✅")        
        ...
        try:
            ...
            user_delete_words.pop(user_id, None)
            user_replacement_words.pop(user_id, None)
...
async def lock_command_handler(event):
    ...
    saved_channel_ids.add(channel_id)
    ...
    try:
        ...
        if freecheck == 1:
            reply_markup = InlineKeyboardMarkup(
                [
                    [InlineKeyboardButton("💎 Get Premium to Forward", url="https://t.me/BADDY_15")]
                ]
            )
