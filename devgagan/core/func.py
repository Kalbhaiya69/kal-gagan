# ---------------------------------------------------
# File Name: func.py
# Description: Helper functions for the bot
# Author: Gagan
    ...
    if update_channel:
        try:
            ...
            await message.reply_photo(
                photo="https://graph.org/file/3eddaf2b77f5e2352640b-1cf1914d750ca43925.jpg",
                caption=caption, 
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Join Now...", url=f"{url}")]])
            )
            return 1
        except Exception:
            await message.reply_text("Something Went Wrong. Contact us @Elexyz...")
            return 1
...
async def prog_bar(current, total, ud_type, message, start):
    ...
    if round(diff % 10.00) == 0 or current == total:
        ...
        try:
            ...
            passimated_total_time != '' else "0 s"
        )
        try:
