...
async def subscribe(app, message):
    ...
    try:
        ...
        if user.status == enums.ChatMemberStatus.BANNED:
            await message.reply_text("You are Banned. Contact -- @Elexyz")
            return 1
    except UserNotParticipant:
        try:
            url = await gen_link(app, update_channel)
        except Exception:
            # Fallback for CHAT_ADMIN_REQUIRED or other issues
            if str(update_channel).startswith("-100"):
                ch_id = str(update_channel).replace("-100", "")
                url = f"https://t.me/c/{ch_id}/1"
            else:
                url = f"https://t.me/{update_channel}"
        
        caption = f"Join our channel to use the bot"
        await message.reply_photo(photo="https://graph.org/file/3eddaf2b77f5e2352640b-1cf1914d750ca43925.jpg",caption=caption, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Join Now...", url=f"{url}")]]))
        return 1
