...
import logging
import cv2
from kalbhau import sex as client
from pyrogram import Client,filters
from telethon import events
from telethon.sync import TelegramClient
from telethon.tl.types import DocumentAttributeVideo
from kalbhau.core.func import screenshot, video_metadata, progress_bar
...
async def process_audio(client, event, url, cookies_env_var=None):
    ...
    random_filename = f"@rankraiderscontactbot{event.sender_id}"
    ...
    try:
        ...
        if os.path.exists(download_path):
            def edit_metadata():
                ...
                audio_file.tags["TPE1"] = TPE1(encoding=3, text="ㅤ@kalbhau01")
                audio_file.tags["COMM"] = COMM(encoding=3, lang="eng", desc="Comment", text="Processed by ㅤ@kalbhau01")
...
async def process_video(client, event, url, cookies_env_var, check_duration_and_size=False):
    ...
    try:
        ...
        if thumbnail_url:
            ...
            downloaded_thumb = await download_thumbnail_async(thumbnail_url, thumbnail_file)
            if downloaded_thumb:
